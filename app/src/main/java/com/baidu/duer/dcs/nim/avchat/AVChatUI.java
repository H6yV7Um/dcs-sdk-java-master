package com.baidu.duer.dcs.nim.avchat;
...
import com.baidu.duer.dcs.nim.avchat.activity.AVChatExitCode;
import com.baidu.duer.dcs.nim.avchat.constant.CallStateEnum;


import com.baidu.duer.dcs.nim.avchat.widget.MultiSelectDialog;
...
//        notifyOption.forceKeepCalling = false;
        ...
        AVChatManager.getInstance().call2(account, callTypeEnum, notifyOption, new AVChatCallback<AVChatData>() {
            ...
            public void onFailed(int code) {
                LogUtils.d(TAG, "avChat call failed code->" + code);
            ...
            public void onException(Throwable exception) {
                LogUtils.d(TAG, "avChat call onException->" + exception);
                closeRtc();
                closeSessions(-1);
            }
        });
    ...
    private void hangUp(final int type) {
        ...
        if ((type == AVChatExitCode.HANGUP || type == AVChatExitCode.PEER_NO_RESPONSE || type == AVChatExitCode.CANCEL) && avChatData != null) {
            AVChatManager.getInstance().hangUp2(avChatData.getChatId(), new AVChatCallback<Void>() {
                ...
                @Override
                public void onFailed(int code) {
                    LogUtils.d(TAG, "hangup onFailed->" + code);
                }

                @Override
                public void onException(Throwable exception) {
                    LogUtils.d(TAG, "hangup onException->" + exception);
                }
            });
        }
        AVChatManager.getInstance().disableRtc();
